...
public:
  ...
  static void print(NodeAVL *root) {
    if (root != nullptr) {
      ...
      cout << "Buku yang dipinjam: " << root->anggota->buku << endl << endl;
      print(root->right);
    }
  }
...
struct Perpustakaan {
  void savePerpus(vector<string> &perpus) {
    ...
    mkdir(folder.c_str());
    file.open(fileDir, ios::out);
    for (auto it = perpus.begin(); it != perpus.end(); ++it) {
      file << perpus.at(i - 1) << endl;
      i++;
    }
    file.close();
  }
  ...
  void createPerpus(vector<string> &perpus) {
    string fileName;
    fileName = validationString("Masukkan nama perpustakaannya: ");
    perpus.push_back(fileName);
    savePerpus(perpus);
    system("cls");
    string folder = "./src";
    mkdir(folder.c_str());
    string fileDir = folder + "/" + fileName + ".txt";
    ofstream file;
    file.open(fileDir, ios::out);
    file.close();
  }
...
int main(int argc, char const *argv[]) {
  ...
  do {
    ...
    if (pilihanChar == 'a' || pilihanChar == 'A') {
      ...
      perpustakaan.createPerpus(perpus);
